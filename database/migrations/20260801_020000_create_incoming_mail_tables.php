...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('incoming_mail_accounts', function (Blueprint $table) {
            $table->id();
            $table->string('label');
            $table->string('protocol')->default('imap'); // imap or pop3
            $table->string('host');
            $table->integer('port')->default(993);
            $table->string('encryption')->default('ssl'); // ssl, tls, none
            $table->string('username');
            $table->text('password'); // store as plain text for IMAP (own credentials)
        ...
        Schema::create('incoming_emails', function (Blueprint $table) {
            ...
            $table->string('status')->default('new'); // new | converted_to_request | converted_to_lead | ignored | failed
