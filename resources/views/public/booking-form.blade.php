...
@section('title', 'Shishi Footsteps Booking Form | Shishi Footsteps')
@section('description', 'Complete your Shishi Footsteps booking details securely online.')
...
<main class="booking-online">
        ...
        <div class="booking-hero__inner">
            ...
            @if($proposal)
                <aside class="booking-card">
                    ...
                    <span>{{ $proposal->guest_count }} guests · {{ $proposal->duration_days }} days</span>
                </aside>
            @endif
        </div>
    </section>
    ...
    <section class="booking-wrap">
        ...
        <form class="booking-form" method="POST" action="{{ route('public.booking.form.submit', $token) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="booking-section">
                ...
                <div class="booking-grid">
                    <div class="booking-field"><label>Full name *</label><input name="main_full_name" value="{{ $clientName }}" required></div>
                    <div class="booking-field"><label>Email *</label><input type="email" name="main_email" value="{{ $clientEmail }}" required></div>
                    <div class="booking-field"><label>Phone</label><input name="main_phone" value="{{ $clientPhone }}"></div>
                    <div class="booking-field"><label>Date of birth</label><input type="date" name="main_date_of_birth" value="{{ old('main_date_of_birth') }}"></div>
                    <div class="booking-field"><label>Sex</label><select name="main_sex"><option value="">Select</option><option @selected(old('main_sex')==='Female')>Female</option><option @selected(old('main_sex')==='Male')>Male</option><option @selected(old('main_sex')==='Prefer not to say')>Prefer not to say</option></select></div>
                    <div class="booking-field"><label>Country</label><input name="country" value="{{ old('country', $proposal->client_country ?? '') }}"></div>
                    <div class="booking-field full"><label>Address</label><input name="address" value="{{ old('address') }}"></div>
                    <div class="booking-field"><label>City</label><input name="city" value="{{ old('city') }}"></div>
                    <div class="booking-field"><label>ZIP code</label><input name="zip_code" value="{{ old('zip_code') }}"></div>
                    <div class="booking-field full"><label>Preferred payment method</label><input name="payment_method" value="{{ old('payment_method') }}" placeholder="Bank transfer, card, Wise, other..."></div>
                </div>
            </div>
            ...
            <div class="booking-section">
                ...
                <div class="booking-grid">
                    <div class="booking-field"><label>Full name</label><input name="emergency_name" value="{{ old('emergency_name') }}"></div>
                    <div class="booking-field"><label>Email</label><input type="email" name="emergency_email" value="{{ old('emergency_email') }}"></div>
                    <div class="booking-field full"><label>Phone</label><input name="emergency_phone" value="{{ old('emergency_phone') }}"></div>
                </div>
            </div>
            ...
            <div class="booking-section">
                ...
                <div class="booking-grid">
                    <div class="booking-field"><label>Arrival date</label><input type="date" name="arrival_date" value="{{ old('arrival_date', $proposal->start_date ?? '') }}"></div>
                    <div class="booking-field"><label>Arrival time</label><input name="arrival_time" value="{{ old('arrival_time') }}" placeholder="e.g. 21:35"></div>
                    <div class="booking-field"><label>Arrival airport / pickup</label><input name="arrival_airport" value="{{ old('arrival_airport') }}"></div>
                    <div class="booking-field"><label>Arrival flight number</label><input name="arrival_flight_number" value="{{ old('arrival_flight_number') }}"></div>
                    <div class="booking-field"><label>Early check-in needed?</label><select name="early_checkin"><option value="no">No</option><option value="yes" @selected(old('early_checkin')==='yes')>Yes</option></select></div>
                    <div></div>
                    <div class="booking-field"><label>Departure date</label><input type="date" name="departure_date" value="{{ old('departure_date', $proposal->end_date ?? '') }}"></div>
                    <div class="booking-field"><label>Departure time</label><input name="departure_time" value="{{ old('departure_time') }}" placeholder="e.g. 23:10"></div>
                    <div class="booking-field"><label>Departure airport / drop-off</label><input name="departure_airport" value="{{ old('departure_airport') }}"></div>
                    <div class="booking-field"><label>Departure flight number</label><input name="departure_flight_number" value="{{ old('departure_flight_number') }}"></div>
                    <div class="booking-field"><label>Late check-out needed?</label><select name="late_checkout"><option value="no">No</option><option value="yes" @selected(old('late_checkout')==='yes')>Yes</option></select></div>
                </div>
            </div>
            ...
            <div class="booking-section">
                ...
                <div class="booking-grid">
                    <div class="booking-field"><label>Flying Doctors</label><select name="flying_doctors"><option value="no">No</option><option value="yes" @selected(old('flying_doctors')==='yes')>Yes</option></select></div>
                    <div class="booking-field"><label>For how many people?</label><input type="number" min="0" name="flying_doctors_people" value="{{ old('flying_doctors_people', 0) }}"></div>
                    <div class="booking-field"><label>Soft drinks and snacks</label><select name="soft_drinks"><option value="no">No</option><option value="yes" @selected(old('soft_drinks')==='yes')>Yes</option></select></div>
                    <div class="booking-field"><label>For how many people?</label><input type="number" min="0" name="soft_drinks_people" value="{{ old('soft_drinks_people', 0) }}"></div>
                    <div class="booking-field"><label>Binoculars</label><select name="binoculars"><option value="no">No</option><option value="yes" @selected(old('binoculars')==='yes')>Yes</option></select></div>
                    <div class="booking-field"><label>How many?</label><input type="number" min="0" name="binoculars_count" value="{{ old('binoculars_count', 0) }}"></div>
                    <div class="booking-field"><label>Baby seats</label><select name="baby_seats"><option value="no">No</option><option value="yes" @selected(old('baby_seats')==='yes')>Yes</option></select></div>
                    <div class="booking-field"><label>How many?</label><input type="number" min="0" name="baby_seats_count" value="{{ old('baby_seats_count', 0) }}"></div>
                </div>
            </div>
            ...
            <div class="booking-section">
                <h2>Anything else?</h2>
                <div class="booking-field full"><label>Notes for Shishi</label><textarea name="notes" placeholder="Dietary requests, rooming notes, medical notes, special celebrations...">{{ old('notes') }}</textarea></div>
            </div>

            <div class="booking-actions">
                <button class="booking-btn" type="submit">Submit booking form</button>
                @if($token)<a class="booking-btn ghost" href="{{ route('proposal.client', $token) }}">Back to proposal</a>@endif
            </div>
        </form>
    </section>
</main>
