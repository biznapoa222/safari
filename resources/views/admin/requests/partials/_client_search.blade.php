<div id="clientDropdown" style="position:relative">
    ...
    <input type="hidden" name="client_id" id="clientId" value="{{ old('client_id', $request->client_id ?? '') }}">
    <input type="hidden" name="client_name" id="clientNameInput" value="{{ old('client_name', $request->client_name ?? '') }}">
    <meta name="search-clients-url" content="{{ route('admin.requests.search-clients') }}">
    <meta name="store-client-url" content="{{ route('admin.requests.store-client') }}">

    <div id="clientSelected" class="{{ old('client_id') ? '' : 'hidden' }}" style="margin-top:8px">
...
<div id="templateSearch" style="margin-top:16px;border-top:1px solid var(--line);padding-top:16px;position:relative">
    ...
    <input type="text" id="templateSearchInput"
           value="{{ old('itinerary_template_name', $initialTemplate?->trip_name ?: $initialTemplate?->name) }}"
    ...
    <input type="hidden" name="itinerary_template_id" id="templateId" value="{{ old('itinerary_template_id', $request->itinerary_template_id ?? '') }}">
    <input type="hidden" name="itinerary_template_name" id="templateName" value="{{ old('itinerary_template_name', $initialTemplate?->trip_name ?: $initialTemplate?->name) }}">
    <meta name="template-search-url" content="{{ route('admin.itinerary-templates.search') }}">
    ...
    <div id="templateSelected" class="{{ $initialTemplate ? '' : 'hidden' }}" style="margin-top:8px">
        <span style="display:inline-flex;align-items:center;gap:6px;background:#dfece0;padding:6px 10px;border-radius:6px;font-size:9px;color:#234A36">
            ...
            <span id="selectedTemplateLabel">{{ trim(($initialTemplate?->trip_name ?: $initialTemplate?->name) . ($initialTemplate?->duration_days ? ' ('.$initialTemplate->duration_days.' days)' : '')) }}</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function autoFillFromTemplate(id) {
        if (!id) return;
        var daysUrl = '{{ route("admin.itinerary-templates.days", ["template" => "TPLID"]) }}'.replace('TPLID', id);
