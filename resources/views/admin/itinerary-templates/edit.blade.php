...
@section('content')
<x-admin.top-bar title="Edit: {{ $template->name }}" description="Itinerary Templates" :search="false" :addButton="false">
    <a href="{{ route('admin.itinerary-templates.show', $template) }}" class="button button-ghost"><i data-lucide="arrow-left"></i> Back</a>
</x-admin.top-bar>

@include('admin.partials.flash')

<form method="POST" action="{{ route('admin.itinerary-templates.update', $template) }}" style="display:grid;grid-template-columns:2fr 1fr;gap:16px;align-items:start">
    @csrf @method('PUT')
        ...
        <section class="ops-panel">
            ...
            <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:12px">
                <div style="grid-column:1/-1">
                    ...
                    <input type="text" name="name" value="{{ old('name', $template->name) }}" style="width:100%;height:38px;padding:0 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">
                </div>
                <div style="grid-column:1/-1">
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Trip Name</label>
                    <input type="text" name="trip_name" value="{{ old('trip_name', $template->trip_name) }}" style="width:100%;height:38px;padding:0 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="status" style="width:100%;height:38px;padding:0 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">
                        ...
                        <option value="{{ $val }}" @selected(old('status', $template->status) === $val)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <section class="ops-panel">
            ...
            <div style="padding:16px;display:flex;flex-direction:column;gap:12px">
                <div>
                    ...
                    <textarea name="overview" rows="4" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('overview', $template->overview) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Highlights</label>
                    <textarea name="highlights" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('highlights', $template->highlights) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Includes</label>
                    <textarea name="includes" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('includes', $template->includes) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Excludes</label>
                    <textarea name="excludes" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('excludes', $template->excludes) }}</textarea>
                </div>
            </div>
        </section>
        ...
        <section class="ops-panel">
            ...
            <div style="padding:16px;display:flex;flex-direction:column;gap:12px">
                <div>
                    ...
                    <textarea name="terms" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('terms', $template->terms) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Booking Terms</label>
                    <textarea name="booking_terms" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('booking_terms', $template->booking_terms) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Payment Schedule</label>
                    <textarea name="payment_schedule" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('payment_schedule', $template->payment_schedule) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Cancellation Policy</label>
                    <textarea name="cancellation_policy" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('cancellation_policy', $template->cancellation_policy) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Refund Policy</label>
                    <textarea name="refund_policy" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('refund_policy', $template->refund_policy) }}</textarea>
                </div>
                <div>
                    <label style="display:block;font-size:9px;font-weight:600;color:var(--text-muted);margin-bottom:6px">Important Notes</label>
                    <textarea name="important_notes" rows="3" style="width:100%;padding:9px 11px;border:1px solid #d9d0c1;border-radius:7px;font-size:9px;background:#F8F5EF;color:var(--text);outline:none;resize:vertical" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='#d9d0c1'">{{ old('important_notes', $template->important_notes) }}</textarea>
                </div>
            </div>
        </section>
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
        <section class="ops-panel" style="position:sticky;top:96px">
            ...
            <div style="padding:16px;display:flex;flex-direction:column;gap:10px">
                ...
                <a href="{{ route('admin.itinerary-templates.show', $template) }}" style="display:block;text-align:center;color:var(--text-muted);font-size:9px;padding:8px 0;text-decoration:none">Cancel</a>
            </div>
        </section>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var dayIndex = 0;
    var existingDays = @json($template->days->toArray());
    var destinations = @json($destinations->map(function($d) { return ['id' => $d->id, 'name' => $d->name]; }));
    var hotelCache = {};
    var activityCache = {};
    ...
    function loadHotels(destId, callback) {
        ...
        var url = '{{ route("admin.hotels.data") }}' + (destId ? '?destination_id=' + destId : '');
    ...
    function loadActivities(destId, callback) {
        ...
        var url = '{{ route("admin.activities.data") }}?destination_id=' + destId;
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(function(r) { return r.json(); })
