<?php $__env->startSection('title', 'Proposals Planning'); ?>
...
<section class="planning-board">
    <header class="planning-header">
        ...
        <nav class="country-tabs" aria-label="Country">
            <?php $__currentLoopData = ['' => 'All countries', 'Tanzania' => 'Tanzania', 'Kenya' => 'Kenya', 'Uganda' => 'Uganda', 'South Africa' => 'South Africa']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a class="<?php echo e(request('country', '') === $value ? 'is-active' : ''); ?>" href="<?php echo e(route('admin.proposal-planning.index', [...request()->except(['page', 'country']), 'country' => $value])); ?>"><?php echo e($label); ?></a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </nav>
        <nav class="stage-tabs" aria-label="Proposal stage">
            <a class="<?php echo e($stage === 'planning' ? 'is-active' : ''); ?>" href="<?php echo e($stageUrl('planning')); ?>">Planning</a>
            <a class="<?php echo e($stage === 'pre-confirmed' ? 'is-active' : ''); ?>" href="<?php echo e($stageUrl('pre-confirmed')); ?>">Pre-confirmed</a>
            <a class="<?php echo e($stage === 'confirmed' ? 'is-active' : ''); ?>" href="<?php echo e($stageUrl('confirmed')); ?>">Confirmed</a>
        </nav>
        <?php if($stage === 'confirmed'): ?>
        <nav class="workflow-tabs" aria-label="Trip stage">
            <a class="<?php echo e($tripTab === 'upcoming' ? 'is-active' : ''); ?>" href="<?php echo e($tripUrl('upcoming')); ?>">Upcoming trips</a>
            <a class="<?php echo e($tripTab === 'in-operation' ? 'is-active' : ''); ?>" href="<?php echo e($tripUrl('in-operation')); ?>">Trips in operation</a>
            <a class="<?php echo e($tripTab === 'operated' ? 'is-active' : ''); ?>" href="<?php echo e($tripUrl('operated')); ?>">Operated trips</a>
            <a class="<?php echo e($tripTab === 'evaluated' ? 'is-active' : ''); ?>" href="<?php echo e($tripUrl('evaluated')); ?>">Evaluated trips</a>
        </nav>
        <?php elseif($stage === 'planning'): ?>
        <nav class="workflow-tabs" aria-label="Planning step">
            <?php $__currentLoopData = ['all' => 'All planning', 'in-planning' => 'In planning by seller', 'quotation-check' => 'Quotation check', 'team-leader-check' => 'Quotation & team leader check', 'done' => 'Done']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a class="<?php echo e($planningStep === $value ? 'is-active' : ''); ?>" href="<?php echo e($stepUrl($value)); ?>"><?php echo e($label); ?></a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </nav>
        <?php endif; ?>
    </header>
    ...
    <form method="GET" class="planning-filters">
        ...
        <?php if($stage === 'confirmed'): ?><input type="hidden" name="trip" value="<?php echo e($tripTab); ?>"><?php endif; ?>
        <?php if($stage === 'planning'): ?><input type="hidden" name="step" value="<?php echo e($planningStep); ?>"><?php endif; ?>
        <label><i data-lucide="search"></i><input type="search" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by request ID, request name or proposal name"></label>
        <label><i data-lucide="user-round"></i><select name="seller"><option value="">Filter by seller</option><?php $__currentLoopData = $sellers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $seller): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($seller->id); ?>" <?php if((string) request('seller') === (string) $seller->id): echo 'selected'; endif; ?>><?php echo e($seller->name); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></label>
        <label><i data-lucide="layers-3"></i><select name="type"><option value="">Itinerary, Custom, Manual, Group</option><?php $__currentLoopData = ['Itinerary','Custom','Manual','Group']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option <?php if(request('type') === $type): echo 'selected'; endif; ?>><?php echo e($type); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></label>
    ...
    <div class="planning-tools">
        ...
        <a class="export-button" href="<?php echo e(route('admin.proposal-planning.export', request()->query())); ?>"><i data-lucide="download"></i>Export Excel / CSV</a>
    </div>
    ...
    <div class="planning-table-wrap">
        <table class="planning-table">
            ...
            <?php $__empty_1 = true; $__currentLoopData = $records; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="payment-row payment-row--<?php echo e($record->payment_state); ?>">
                    <td><a href="<?php echo e(route('admin.quotations.show', $record->id)); ?>"><?php echo e($record->reference); ?></a><small><?php if($record->client_token): ?><a class="client-preview-link" href="<?php echo e(route('proposal.client', $record->client_token)); ?>" target="_blank" title="Open client proposal"><?php echo e($record->client_name); ?> ↗</a><?php else: ?><?php echo e($record->client_name); ?><?php endif; ?></small></td>
                    <td><a href="<?php echo e(route('admin.quotations.show', $record->id)); ?>">Open: <?php echo e($record->title); ?></a><small><?php echo e($record->duration_days); ?> days · <?php echo e($record->guest_count); ?> guests</small></td>
                    <td><span class="seller-dot" style="--dot-hue:<?php echo e(($record->seller_id ?? 1) * 47 % 360); ?>"></span><?php echo e($record->seller_name); ?></td>
                    <td><?php echo e(\Carbon\Carbon::parse($record->start_date)->format('d-m-Y')); ?></td>
                    <td><?php echo e(\Carbon\Carbon::parse($record->end_date)->format('d-m-Y')); ?></td>
                    <td><?php echo e($record->reservation_person); ?><small><?php echo e($record->confirmed_reservations); ?>/<?php echo e($record->reservation_count); ?> confirmed</small></td>
                    <td><span class="state-pill state-pill--<?php echo e($record->reservation_state === 'Confirmed' ? 'green' : ($record->reservation_state === 'New' ? 'yellow' : 'blue')); ?>"><?php echo e($record->reservation_state); ?></span></td>
                    <td><span class="bool-pill <?php echo e($record->days_complete ? 'is-yes' : ''); ?>"><?php echo e($record->days_complete ? '✓' : 'No'); ?></span></td>
                    <td><span class="bool-pill">No</span></td>
                    <?php $__currentLoopData = ['jeeps_planned_at','daily_movements_checked_at','pre_departure_checked_at']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <td>
                        <form method="POST" action="<?php echo e(route('admin.proposal-planning.toggle', $record->id)); ?>"><?php echo csrf_field(); ?><input type="hidden" name="field" value="<?php echo e($field); ?>"><button class="bool-pill <?php echo e($record->{$field} ? 'is-yes' : 'is-no'); ?>" title="Toggle checklist"><?php echo e($record->{$field} ? '✓' : '⊘'); ?></button></form>
                    </td>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <td><?php echo e($record->proposal_type); ?></td>
                    <td>
                        <details class="planning-note"><summary><?php echo e($record->planning_note ?: 'ADD NOTE'); ?></summary><form method="POST" action="<?php echo e(route('admin.proposal-planning.note', $record->id)); ?>"><?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?><textarea name="planning_note" rows="3" placeholder="Planning note"><?php echo e($record->planning_note); ?></textarea><input name="whatsapp_status" value="<?php echo e($record->whatsapp_status); ?>" placeholder="WhatsApp status"><button>Save note</button></form></details>
                    </td>
                    <td>
                        <?php if($record->stage !== 'confirmed'): ?>
                        <form method="POST" action="<?php echo e(route('admin.proposal-planning.advance', $record->id)); ?>"><?php echo csrf_field(); ?><button class="advance-button">Advance <i data-lucide="arrow-right"></i></button></form>
                        <?php elseif($record->trip_stage === 'evaluated'): ?>
                        <a class="advance-button" href="<?php echo e(route('admin.evaluations.show', $record->id)); ?>">Evaluation <i data-lucide="arrow-up-right"></i></a>
                        <?php else: ?>
                        <a class="advance-button" href="<?php echo e(route('admin.quotations.show', $record->id)); ?>">Open <i data-lucide="arrow-up-right"></i></a>
                        <?php endif; ?>
                    </td>
                </tr>
    ...
    <div class="planning-pagination"><?php echo e($records->links()); ?></div>
</section>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\user\Documents\xamp\htdocs\safari\resources\views\admin\proposal-planning\index.blade.php ENDPATH**/ ?>